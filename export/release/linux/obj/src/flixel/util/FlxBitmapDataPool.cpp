...
HXLINE(  56)					node->prev->_hx_set_next(HX_CTX, node->next);
            				}
HXLINE(  57)				if (::hx::IsNotNull( node->next )) {
HXLINE(  58)					node->next->_hx_set_prev(HX_CTX, node->prev);
            				}
...
HXLINE( 100)				last->prev->_hx_set_next(HX_CTX, null());
...
HXLINE( 108)		node->_hx_set_next(HX_CTX, ::flixel::util::FlxBitmapDataPool_obj::_head);
...
HXLINE( 116)			node->next->_hx_set_prev(HX_CTX, node);
            		}
